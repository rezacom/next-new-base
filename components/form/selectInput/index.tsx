...
export default function SelectInput(props: BaseSelectProps) {
  ...
  const [textValue, setTextValue] = useState<string | undefined>(props.defaultValue);

  const [show, setShow] = useState<boolean>(false);
  const wrapperRef: React.MutableRefObject<any> = useRef(null);

  useEffect(() => {
    const result = props.items?.find((item: any) => item.value == props.selectedKey);
    setValue(result);
  ...
  const onChangeValue = (e: { target: { value: React.SetStateAction<string> } }) => {
    // props.handleChangeValue && props.handleChangeValue(e.target.value as string);
  ...
  const handleShow = () => {
    ...
    if (props.hasSearch && props.handleChangeValue && !textValue) props.handleChangeValue("");
  };

  return (
    <div
      className={clsx(`w-full h-max`, props.classNames?.base as string, props.className)}
      ref={wrapperRef}
      dir={props.dir}
    >
      {props.label ? (
        <label className={clsx("dark:text-white-light", props.classNames?.label as string)} onClick={handleShow}>
          {props.label}
        </label>
      ) : (
        <></>
      ...
      >
        ...
        {value?.icon ? <img src={value.icon} className={clsx("w-5 h-5 rounded-3xl", props.classNames?.icon)} /> : <></>}
        {!props.hideValue && (
          <input
            name={props.name}
            readOnly
        ...
        <div
          className={clsx(
            ...
            show ? "visible opacity-100 top-[110%]" : "opacity-0 invisible top-[130%]",
              ...
              return (
                <li
                  ...
                  className={clsx(
                    ...
                    value?.value === item?.value ? "text-[#4361ee] bg-[#4361ee1a]" : ""
                  )}
                >
                  {item?.icon ? <img src={item.icon} className="w-5 h-5 rounded-3xl" /> : <></>}
                  {item?.label ? <span className="text-small font-normal truncate grow">{item.label}</span> : <></>}
                  {/* <ReactSVG
      ...
      {props.errorMessage && <p className={props.classNames?.errorMessage as string}>{props.errorMessage as string}</p>}
    </div>
  );
}
